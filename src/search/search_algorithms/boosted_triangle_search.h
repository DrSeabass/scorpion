...
/*
  ...
  lists, a helpful list is a sparse subset of the layer's live content, not
  a full copy of it. preferred_evals=[] (default) adds no helpful lists and
  is an exact no-op reduction to today's behavior. Preferred-operator sets
  are computed once per state, when it is first evaluated as a successor
  (see evaluate_and_prepare_node), and persisted until that state is popped
  for expansion (see step()) -- so no evaluator is ever run a second time
  just to learn which of its operators are preferred, unlike naive
  expansion-time re-evaluation.

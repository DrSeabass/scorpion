#ifndef SEARCH_ALGORITHMS_RATCHET_BOOSTED_SWEEP_TRIANGLE_SEARCH_H
#define SEARCH_ALGORITHMS_RATCHET_BOOSTED_SWEEP_TRIANGLE_SEARCH_H
...
/*
  ...
  (combos 1a,2d / 1b,2d) with the ratchet_triangle_search slope-adjustment
  mechanism ported on as-is and always on ("must have" -- no option to
  disable it). The cascade bound each step is max_active_layer + slope,
  where slope is a persistent state variable doubled or halved at the end
  of every step based on that step's heuristic-trend balance (strictly
  more informed than uninformed layer-transitions -> double; otherwise
  halve, floor 1). This trend signal is a single step-scoped counter over
  every expansion in the step regardless of which guidance list served it
  -- orthogonal to the progress-credit budgets below (per-layer or
  global), which decide the dive's served list once, before the cascade
  loop.
  ...
  boosted_sweep_triangle_search is that slope now evolves instead of
  staying fixed. See ratchet_boosted_triangle_search for the per-layer
  reselection sibling.
  ...
  ratchet slope mechanism above: helpful lists affect which list is
  served, the ratchet affects how many layers the cascade covers.
...
*/
class RatchetBoostedSweepTriangleSearch : public SearchAlgorithm {
    struct OpenEntry {
        StateID id;
        int h;
        int g;
    ...
    // boosted_sweep_triangle_search (slope still ratchets regardless).
...
public:
    RatchetBoostedSweepTriangleSearch(
    ...
    virtual ~RatchetBoostedSweepTriangleSearch() = default;

...
/*
  ...
  of every step based on that step's heuristic-trend balance (strictly more
  informed than uninformed layer-transitions -> double; otherwise halve,
  floor 1). This trend signal is a single step-scoped counter over every
  expansion in the step regardless of which guidance list served it --
  orthogonal to the progress-credit budgets below (per-layer or global),
  which track each list's own trend independently and decide the dive's
  served list once, before the cascade loop.

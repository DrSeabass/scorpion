...
BeamSearch::BeamSearch(
    ...
    if (beam_width <= 0) {
        cerr << "BeamSearch: beam_width must be positive." << endl;
        utils::exit_with(utils::ExitCode::SEARCH_INPUT_ERROR);
    }
}

void BeamSearch::initialize() {
    log << "Conducting eager beam search with beam width " << beam_width
...
void BeamSearch::print_statistics() const {
...
SearchStatus BeamSearch::step() {
    ...
    if (next_beam.empty()) {
        log << "Beam became empty after pruning -- no solution!" << endl;
        return FAILED;
    }
...
void BeamSearch::start_evaluator_statistics(EvaluationContext &eval_context) {

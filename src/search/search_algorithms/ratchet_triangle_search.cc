...
RatchetTriangleSearch::RatchetTriangleSearch(
    ...
    if (slope <= 0) {
        cerr << "RatchetTriangleSearch: initial slope must be positive." << endl;
...
void RatchetTriangleSearch::print_statistics() const {
    statistics.print_detailed_statistics();
    search_space.print_statistics();
    pruning_method->print_statistics();
}

void RatchetTriangleSearch::start_evaluator_statistics(EvaluationContext &eval_context) {
...
void RatchetTriangleSearch::extend_open_lists(int num_lists) {
    for (int i = 0; i < num_lists; ++i) {
        open_lists.emplace_back();
    }
}

void RatchetTriangleSearch::recompute_max_active_layer() {
    while (max_active_layer >= 0 && open_lists[max_active_layer].empty()) {
        --max_active_layer;
    }
}

void RatchetTriangleSearch::update_incumbent(const State &goal_state) {
    Plan candidate_plan =
...
bool RatchetTriangleSearch::evaluate_and_prepare_node(
...
void RatchetTriangleSearch::insert_into_open_list(int list_index, const OpenEntry &entry) {
    assert(list_index >= 0);

...
namespace ratchet_boosted_sweep_triangle_search {

RatchetBoostedSweepTriangleSearch::RatchetBoostedSweepTriangleSearch(
    ...
    if (slope <= 0) {
        cerr << "RatchetBoostedSweepTriangleSearch: initial slope must be positive." << endl;
        utils::exit_with(utils::ExitCode::SEARCH_INPUT_ERROR);
    }
    if (evals.empty()) {
        cerr << "RatchetBoostedSweepTriangleSearch: at least one guidance evaluator is required." << endl;
        utils::exit_with(utils::ExitCode::SEARCH_INPUT_ERROR);
    }
    ...
    for (const shared_ptr<Evaluator> &pref_eval : preferred_evals) {
        ...
        if (source == -1) {
            cerr << "RatchetBoostedSweepTriangleSearch: every preferred_evals "
...
void RatchetBoostedSweepTriangleSearch::initialize() {
    log << "Conducting ratchet-boosted-sweep triangle search with initial slope " << slope
...
void RatchetBoostedSweepTriangleSearch::print_statistics() const {
    ...
    if (log.is_at_least_debug()) {
        if (credit_scope == CreditScope::GLOBAL) {
            log << "Ratchet-boosted-sweep triangle global final budgets:";
            for (int k = 0; k < total_lists; ++k)
                log << " " << global_progress[k].budget;
            log << endl;
        }
        for (size_t i = 0; i < layers.size(); ++i) {
            log << "Ratchet-boosted-sweep triangle layer " << (depth_offset + static_cast<int>(i))
                << " final budgets:";
            for (int k = 0; k < total_lists; ++k)
...
void RatchetBoostedSweepTriangleSearch::start_evaluator_statistics(EvaluationContext &eval_context) {
...
void RatchetBoostedSweepTriangleSearch::extend_layers(int num_layers) {
    for (int i = 0; i < num_layers; ++i) {
        layers.emplace_back(total_lists);
    }
}

bool RatchetBoostedSweepTriangleSearch::layer_empty(int layer) const {
...
void RatchetBoostedSweepTriangleSearch::recompute_max_active_layer() {
    while (max_active_layer >= 0 && layer_empty(max_active_layer)) {
        --max_active_layer;
    }
}

void RatchetBoostedSweepTriangleSearch::update_incumbent(const State &goal_state) {
    Plan candidate_plan =
    ...
    if (!found_solution() || candidate_cost < bound) {
        ...
        log << "RatchetBoostedSweepTriangleSearch: improved incumbent with cost " << candidate_cost << endl;
        if (anytime_search) {
...
bool RatchetBoostedSweepTriangleSearch::evaluate_and_prepare_node(
...
void RatchetBoostedSweepTriangleSearch::cache_preferred_operators(
...
int RatchetBoostedSweepTriangleSearch::select_credit_served(
...
int RatchetBoostedSweepTriangleSearch::select_sweep_served(int round_robin_served) const {
...
int RatchetBoostedSweepTriangleSearch::select_available_served(
    const Layer &layer, int intended) const {
    // ICAPS-27 step 6: walk the round-robin order (wrapping) from the
    // intended list until a non-empty one is found at this layer, rather
    // than re-litigating the once-per-dive (or per-layer round-robin)
    // policy decision. At preferred_evals=[] this always returns `intended`
    // itself on the very first check (every guidance/pruner list shares
    // identical live content, so intended's own list can only be empty
    // when every list is) -- an exact reduction to the pre-step-6 behavior.
...
void RatchetBoostedSweepTriangleSearch::record_expansion_credit(
...
void RatchetBoostedSweepTriangleSearch::insert_successor(
...
SearchStatus RatchetBoostedSweepTriangleSearch::step() {
    while (!layers.empty() && layer_empty(0)) {
        if (log.is_at_least_debug()) {
            log << "RatchetBoostedSweepTriangleSearch: layer " << depth_offset
    ...
    // The cascade bound is the step-start max_active_layer plus the current
    // (ratcheted) slope; the deque grows lazily as successors get inserted,
    // rather than being pre-extended by slope at the top of step().
    ...
    // layer is live-empty -- see select_available_served for how step 6's
    // sparser helpful lists changed that reasoning.
    ...
    // ratchet_triangle_search, always on -- see icaps-27-plan.md step 5).
    // This is a single step-scoped trend counter over every expansion in
    // the step, regardless of which guidance list served it -- orthogonal
    // to the progress-credit budgets above, and unaffected by step 6's
    // helpful lists (it only ever looks at the h of whichever state got
    // expanded, not which list served it).
    ...
    for (int i = 0; i < cascade_cap; ++i) {
        ...
        // ICAPS-27 step 6: the intended list (the once-per-dive policy pick,
        // or the per-layer round-robin candidate) can be selectively empty
        // at this specific layer once helpful lists exist -- fall back to
        // the nearest non-empty list rather than treating that as "the
        // whole layer is empty."
        const int served = select_available_served(layers[i], intended);
        ...
        // Ratchet slope-adjustment trend signal (see above): compares this

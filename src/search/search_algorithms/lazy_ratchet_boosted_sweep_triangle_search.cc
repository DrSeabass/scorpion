...
LazyRatchetBoostedSweepTriangleSearch::LazyRatchetBoostedSweepTriangleSearch(
    ...
    : SearchAlgorithm(cost_type, bound, max_time, description, verbosity),
      ...
      root_pending(true) {
    if (slope <= 0) {
        cerr << "LazyRatchetBoostedSweepTriangleSearch: initial slope must be positive." << endl;
        utils::exit_with(utils::ExitCode::SEARCH_INPUT_ERROR);
    }
    if (evals.empty()) {
        cerr << "LazyRatchetBoostedSweepTriangleSearch: at least one guidance evaluator is required." << endl;
        utils::exit_with(utils::ExitCode::SEARCH_INPUT_ERROR);
    }
    ...
    for (const shared_ptr<Evaluator> &pref_eval : preferred_evals) {
        ...
        if (source == -1) {
            cerr << "LazyRatchetBoostedSweepTriangleSearch: every preferred_evals "
...
void LazyRatchetBoostedSweepTriangleSearch::initialize() {
    log << "Conducting lazy ratchet-boosted-sweep triangle search with initial slope " << slope
...
void LazyRatchetBoostedSweepTriangleSearch::print_statistics() const {
    ...
    if (log.is_at_least_debug()) {
        if (credit_scope == CreditScope::GLOBAL) {
            log << "Lazy ratchet-boosted-sweep triangle global final budgets:";
            for (int k = 0; k < total_lists; ++k)
                log << " " << global_progress[k].budget;
            log << endl;
        }
        for (size_t i = 0; i < layers.size(); ++i) {
            log << "Lazy ratchet-boosted-sweep triangle layer " << (depth_offset + static_cast<int>(i))
                << " final budgets:";
            for (int k = 0; k < total_lists; ++k)
...
void LazyRatchetBoostedSweepTriangleSearch::start_evaluator_statistics(EvaluationContext &eval_context) {
...
bool LazyRatchetBoostedSweepTriangleSearch::has_non_empty_lists() const {
    if (root_pending)
        return true;
...
bool LazyRatchetBoostedSweepTriangleSearch::layer_empty(const Layer &layer) const {
...
LazyRatchetBoostedSweepTriangleSearch::Layer LazyRatchetBoostedSweepTriangleSearch::create_layer() const {
...
void LazyRatchetBoostedSweepTriangleSearch::extend_layers(int num_layers) {
    for (int i = 0; i < num_layers; ++i) {
        layers.push_back(create_layer());
    }
}

void LazyRatchetBoostedSweepTriangleSearch::trim_empty_layers() {
...
int LazyRatchetBoostedSweepTriangleSearch::select_credit_served(
...
int LazyRatchetBoostedSweepTriangleSearch::select_sweep_served(int round_robin_served) const {
...
int LazyRatchetBoostedSweepTriangleSearch::select_available_served(
...
void LazyRatchetBoostedSweepTriangleSearch::record_expansion_credit(
...
void LazyRatchetBoostedSweepTriangleSearch::update_incumbent(const State &goal_state) {
    Plan candidate_plan =
    ...
    if (!found_solution() || candidate_cost < bound) {
        ...
        log << "LazyRatchetBoostedSweepTriangleSearch: improved incumbent with cost " << candidate_cost << endl;
        if (anytime_search) {
...
LazyRatchetBoostedSweepTriangleSearch::ExpansionOutcome LazyRatchetBoostedSweepTriangleSearch::process_candidate(
    const State &state,
...
SearchStatus LazyRatchetBoostedSweepTriangleSearch::step() {
    ...
    // Ratchet slope-adjustment bookkeeping (ported as-is, always on),
    // orthogonal to the once-per-dive credit_scope/credit_boost selection
    // above: a single step-scoped trend counter over every expansion in
    // the step, regardless of which guidance list served it.
    ...
    for (int i = 0; i < num_layers - 1; ++i) {
        ...
        while (!list.empty() && !expanded) {
            ...
            if (expanded) {
                ...
                // Ratchet slope-adjustment trend signal (see above).
                if (have_last_h) {
    ...
    // uninformed transitions, otherwise halve (floor 1).

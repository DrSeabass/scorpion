...
BoostedTriangleSearch::BoostedTriangleSearch(
    ...
    for (const shared_ptr<Evaluator> &pref_eval : preferred_evals) {
        ...
        if (source == -1) {
            cerr << "BoostedTriangleSearch: every preferred_evals entry must "
...
void BoostedTriangleSearch::cache_preferred_operators(
...
int BoostedTriangleSearch::select_round_robin_served(
    const Layer &layer, int round_robin_served) const {
...
SearchStatus BoostedTriangleSearch::step() {
    ...
    // heuristics. Because every live state is inserted into all N guidance
    // (and pruner) lists, the served list being (live-)empty at a layer used
    // to mean the layer was live-empty -- see select_round_robin_served /

...
RatchetBoostedTriangleSearch::RatchetBoostedTriangleSearch(
    ...
    if (slope <= 0) {
        cerr << "RatchetBoostedTriangleSearch: initial slope must be positive." << endl;
        utils::exit_with(utils::ExitCode::SEARCH_INPUT_ERROR);
    }
    if (evals.empty()) {
        cerr << "RatchetBoostedTriangleSearch: at least one guidance evaluator is required." << endl;
        utils::exit_with(utils::ExitCode::SEARCH_INPUT_ERROR);
    }
    ...
    for (const shared_ptr<Evaluator> &pref_eval : preferred_evals) {
        ...
        if (source == -1) {
            cerr << "RatchetBoostedTriangleSearch: every preferred_evals entry "
...
void RatchetBoostedTriangleSearch::initialize() {
    log << "Conducting ratchet-boosted triangle search with initial slope " << slope
...
void RatchetBoostedTriangleSearch::print_statistics() const {
    ...
    if (log.is_at_least_debug()) {
        for (size_t i = 0; i < layers.size(); ++i) {
            log << "Ratchet-boosted triangle layer " << (depth_offset + static_cast<int>(i))
                << " final budgets:";
...
void RatchetBoostedTriangleSearch::start_evaluator_statistics(EvaluationContext &eval_context) {
...
void RatchetBoostedTriangleSearch::extend_layers(int num_layers) {
    for (int i = 0; i < num_layers; ++i) {
        layers.emplace_back(total_lists);
    }
}

bool RatchetBoostedTriangleSearch::layer_empty(int layer) const {
...
void RatchetBoostedTriangleSearch::recompute_max_active_layer() {
    while (max_active_layer >= 0 && layer_empty(max_active_layer)) {
        --max_active_layer;
    }
}

void RatchetBoostedTriangleSearch::update_incumbent(const State &goal_state) {
    Plan candidate_plan =
    ...
    if (!found_solution() || candidate_cost < bound) {
        ...
        log << "RatchetBoostedTriangleSearch: improved incumbent with cost " << candidate_cost << endl;
        if (anytime_search) {
...
bool RatchetBoostedTriangleSearch::evaluate_and_prepare_node(
...
void RatchetBoostedTriangleSearch::cache_preferred_operators(
...
int RatchetBoostedTriangleSearch::select_credit_served(
...
int RatchetBoostedTriangleSearch::select_round_robin_served(
    const Layer &layer, int round_robin_served) const {
...
void RatchetBoostedTriangleSearch::record_expansion_credit(
...
void RatchetBoostedTriangleSearch::insert_successor(
...
SearchStatus RatchetBoostedTriangleSearch::step() {
    while (!layers.empty() && layer_empty(0)) {
        if (log.is_at_least_debug()) {
            log << "RatchetBoostedTriangleSearch: layer " << depth_offset
    ...
    // to the per-list progress-credit budgets above, and unaffected by
    // step 6's helpful lists (it only ever looks at the h of whichever
    // state got expanded, not which list served it).

...
LazyBoostedSweepTriangleSearch::LazyBoostedSweepTriangleSearch(
    ...
    : SearchAlgorithm(cost_type, bound, max_time, description, verbosity),
      ...
      root_pending(true) {
    if (slope <= 0) {
        cerr << "LazyBoostedSweepTriangleSearch: slope must be positive." << endl;
        utils::exit_with(utils::ExitCode::SEARCH_INPUT_ERROR);
    }
    if (evals.empty()) {
        cerr << "LazyBoostedSweepTriangleSearch: at least one guidance evaluator is required." << endl;
        utils::exit_with(utils::ExitCode::SEARCH_INPUT_ERROR);
    }
    ...
    for (const shared_ptr<Evaluator> &pref_eval : preferred_evals) {
        ...
        if (source == -1) {
            cerr << "LazyBoostedSweepTriangleSearch: every preferred_evals entry must "
...
void LazyBoostedSweepTriangleSearch::initialize() {
    log << "Conducting lazy multi-heuristic (sweep) triangle search with slope " << slope
...
void LazyBoostedSweepTriangleSearch::print_statistics() const {
    statistics.print_detailed_statistics();
    search_space.print_statistics();
    pruning_method->print_statistics();

    // ICAPS-27 progress credit (axis 1): final budgets.
    if (log.is_at_least_debug()) {
        if (credit_scope == CreditScope::GLOBAL) {
            log << "Lazy boosted-sweep triangle global final budgets:";
            for (int k = 0; k < total_lists; ++k)
                log << " " << global_progress[k].budget;
            log << endl;
        }
        for (size_t i = 0; i < layers.size(); ++i) {
            log << "Lazy boosted-sweep triangle layer " << (depth_offset + static_cast<int>(i))
                << " final budgets:";
            for (int k = 0; k < total_lists; ++k)
...
void LazyBoostedSweepTriangleSearch::start_evaluator_statistics(EvaluationContext &eval_context) {
...
bool LazyBoostedSweepTriangleSearch::has_non_empty_lists() const {
    if (root_pending)
        return true;
...
bool LazyBoostedSweepTriangleSearch::layer_empty(const Layer &layer) const {
...
LazyBoostedSweepTriangleSearch::Layer LazyBoostedSweepTriangleSearch::create_layer() const {
...
void LazyBoostedSweepTriangleSearch::extend_layers(int num_layers) {
    for (int i = 0; i < num_layers; ++i) {
        layers.push_back(create_layer());
    }
}

void LazyBoostedSweepTriangleSearch::trim_empty_layers() {
...
int LazyBoostedSweepTriangleSearch::select_credit_served(
...
int LazyBoostedSweepTriangleSearch::select_sweep_served(int round_robin_served) const {
    ...
    // cascade wins the whole dive, not just its shallowest or deepest layer.
...
int LazyBoostedSweepTriangleSearch::select_available_served(
    ...
    // itself on the very first check.
...
void LazyBoostedSweepTriangleSearch::record_expansion_credit(
...
void LazyBoostedSweepTriangleSearch::update_incumbent(const State &goal_state) {
    Plan candidate_plan =
    ...
    if (!found_solution() || candidate_cost < bound) {
        ...
        log << "LazyBoostedSweepTriangleSearch: improved incumbent with cost " << candidate_cost << endl;
        if (anytime_search) {
...
LazyBoostedSweepTriangleSearch::ExpansionOutcome LazyBoostedSweepTriangleSearch::process_candidate(
    const State &state,
...
SearchStatus LazyBoostedSweepTriangleSearch::step() {
    ...
    // ICAPS-27 axis 2d (see icaps-27-plan.md): the served list for the
    // whole dive is decided once here, before the cascade loop, and held
    // fixed for every layer below. credit_scope == GLOBAL (combo 1b,2d)
    // reads the single shared budget directly; PER_LAYER (combo 1a,2d)
    // sums budgets across every layer currently active first. credit_boost
    // == 0 skips this entirely.
    int locked_served = -1;
    if (credit_boost != 0) {
    ...
    for (int i = 0; i < num_layers - 1; ++i) {
        ...
        while (!list.empty() && !expanded) {
            ...
            if (expanded) {
                ...
                // for SWEEP, and no effect at all once credit locks the
                // served list for the whole dive.
                ++pop_count;
            }
        }
    }

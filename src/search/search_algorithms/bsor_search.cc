...
BSORSearch::BSORSearch(
    ...
    : SearchAlgorithm(cost_type, bound, max_time, description, verbosity),
      ...
      next_seq(0) {
    if (w < 1.0) {
        cerr << "BSORSearch: suboptimality bound w must be >= 1." << endl;
        utils::exit_with(utils::ExitCode::SEARCH_INPUT_ERROR);
    }
    if (aspect <= 0.0) {
        cerr << "BSORSearch: aspect must be positive." << endl;
        utils::exit_with(utils::ExitCode::SEARCH_INPUT_ERROR);
    }
}

void BSORSearch::initialize() {
    log << "Conducting " << (round_robin ? "round-robin " : "")
...
void BSORSearch::print_statistics() const {
    statistics.print_detailed_statistics();
    search_space.print_statistics();
    pruning_method->print_statistics();
}

void BSORSearch::start_evaluator_statistics(EvaluationContext &eval_context) {
...
int BSORSearch::assign_seq(const State &state) {
    int seq = next_seq++;
    node_seq[state] = seq;
...
void BSORSearch::ensure_level(int idx) {
...
bool BSORSearch::has_non_empty_rect() const {
...
// Insert a frontier node into both open (keyed on f) and its rectangle depth
// bucket (keyed on d). The invariant is that a node lives in both or neither.
void BSORSearch::frontier_insert(const State &state) {
    int seq = node_seq[state];
    int de = node_de[state];
    int g = search_space.get_node(state).get_g();
    open.insert({g + node_h[state], seq});
    ensure_level(de);
    rect[de].insert({node_d[state], seq});
}

// Remove a frontier node from both structures. Must be called with the node's
// current g/de (i.e. before those are changed) so the stored keys match.
void BSORSearch::frontier_erase(const State &state) {
    int seq = node_seq[state];
    int de = node_de[state];
    int g = search_space.get_node(state).get_g();
    open.erase({g + node_h[state], seq});
    rect[de].erase({node_d[state], seq});
}

// NEXT (Algorithm 4, lines 24-29): advance `iteration`/`level` to the next
// rectangle cell that is eligible for expansion, or report that none remain.
bool BSORSearch::advance_rectangle() {
    while (true) {
...
// EXPAND (Algorithm 5). `state` has already been removed from open and rect.
void BSORSearch::expand(const State &state) {
...
bool BSORSearch::try_improve_incumbent(
    ...
    set_plan(plan);
    bound = cost;
    log << "BSOR: improved incumbent with cost " << cost << endl;
    return true;
}

SearchStatus BSORSearch::step() {
    ...
    // within the suboptimality bound, i.e. w * f_min >= g(incumbent).
    if (found_solution()) {
        int f_min = open.begin()->first;
        if (w * static_cast<double>(f_min) >= static_cast<double>(bound)) {
    ...
    // Rectangle expansion (Algorithm 4, lines 14-22).
    ...
    while (!have_node) {
        ...
        SearchNode n_node = search_space.get_node(n);
        open.erase({n_node.get_g() + node_h[n], entry.second});

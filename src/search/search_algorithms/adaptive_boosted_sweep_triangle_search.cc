...
AdaptiveBoostedSweepTriangleSearch::AdaptiveBoostedSweepTriangleSearch(
    ...
    if (evals.empty()) {
        cerr << "AdaptiveBoostedSweepTriangleSearch: at least one guidance evaluator is required." << endl;
    ...
    for (const shared_ptr<Evaluator> &pref_eval : preferred_evals) {
        ...
        if (source == -1) {
            cerr << "AdaptiveBoostedSweepTriangleSearch: every preferred_evals "
...
void AdaptiveBoostedSweepTriangleSearch::initialize() {
    log << "Conducting adaptive-boosted-sweep triangle search with "
...
void AdaptiveBoostedSweepTriangleSearch::print_statistics() const {
    ...
    if (log.is_at_least_debug()) {
        if (credit_scope == CreditScope::GLOBAL) {
            log << "Adaptive-boosted-sweep triangle global final budgets:";
            for (int k = 0; k < total_lists; ++k)
                log << " " << global_progress[k].budget;
            log << endl;
        }
        for (size_t i = 0; i < layers.size(); ++i) {
            log << "Adaptive-boosted-sweep triangle layer " << (depth_offset + static_cast<int>(i))
                << " final budgets:";
            for (int k = 0; k < total_lists; ++k)
...
void AdaptiveBoostedSweepTriangleSearch::start_evaluator_statistics(EvaluationContext &eval_context) {
...
void AdaptiveBoostedSweepTriangleSearch::extend_layers(int num_layers) {
    for (int i = 0; i < num_layers; ++i) {
        layers.emplace_back(total_lists);
    }
}

bool AdaptiveBoostedSweepTriangleSearch::layer_empty(int layer) const {
...
void AdaptiveBoostedSweepTriangleSearch::recompute_max_active_layer() {
    while (max_active_layer >= 0 && layer_empty(max_active_layer)) {
        --max_active_layer;
    }
}

void AdaptiveBoostedSweepTriangleSearch::update_incumbent(const State &goal_state) {
    Plan candidate_plan =
    ...
    if (!found_solution() || candidate_cost < bound) {
        ...
        log << "AdaptiveBoostedSweepTriangleSearch: improved incumbent with cost " << candidate_cost << endl;
        if (anytime_search) {
...
bool AdaptiveBoostedSweepTriangleSearch::evaluate_and_prepare_node(
...
void AdaptiveBoostedSweepTriangleSearch::cache_preferred_operators(
...
int AdaptiveBoostedSweepTriangleSearch::select_credit_served(
...
int AdaptiveBoostedSweepTriangleSearch::select_sweep_served(int round_robin_served) const {
...
int AdaptiveBoostedSweepTriangleSearch::select_available_served(
...
void AdaptiveBoostedSweepTriangleSearch::record_expansion_credit(
...
void AdaptiveBoostedSweepTriangleSearch::insert_successor(
...
SearchStatus AdaptiveBoostedSweepTriangleSearch::step() {
    while (!layers.empty() && layer_empty(0)) {
        if (log.is_at_least_debug()) {
            log << "AdaptiveBoostedSweepTriangleSearch: layer " << depth_offset
    ...
    // it -- orthogonal to the progress-credit budgets above. last_expanded_h
    // is preserved across skipped empty layers.

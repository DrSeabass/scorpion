#!/usr/bin/env python3
"""
Adaptive triangle (g-only pruning) with non_progress_penalty=0.

Direction-A lab follow-up to 2026-05-26-B-adaptive-vs-static-vs-lama.py: the
B-experiment showed adaptive consistently below ratchet, and the hypothesis is
that adaptive's symmetric +1/-1 budget rule lets a single uninformed
h-transition halt mid-step forward progress. This experiment isolates that
hypothesis with the cheapest knob: zero the debit, keep the refund.

  adaptive-triangle-gonly-np0:
      adaptive_triangle(eval=ff(), anytime=true, non_progress_penalty=0)

That single config is the entire suite -- the FF-guided gonly baseline from
the B-experiment is the natural comparison line (same suite, same budget,
same memory, same eval), to be stitched in during analysis.
...
if _domains_filter:
    ...
    SUITE = requested
print(f"[adaptive-triangle-np0] {len(SUITE)} domains under {BENCHMARKS_DIR}")
...
def build_limited_suite(benchmarks_dir, domains, instances_per_domain, instance_step):
    ...
    print(
        f"[adaptive-triangle-np0] {len(tasks)} tasks "
...
if IS_TETRALITH:
    ...
    print(
        f"[adaptive-triangle-np0] {NUM_RUNS} runs, {RUNS_PER_TASK} runs/array-task; "

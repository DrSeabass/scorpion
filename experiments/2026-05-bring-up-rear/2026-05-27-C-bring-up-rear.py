...
Environment variables (defaults shown; Tetralith overrides marked):
  ...
  LIFT_FLOOR_MEMORY                  default 8G
  LIFT_FLOOR_PROCESSES               local-only, default 2
...
BENCHMARK_TARGET = os.environ.get(
    "LIFT_FLOOR_BENCHMARK_TARGET", BENCHMARK_TARGET_DEFAULT
)
if BENCHMARK_TARGET not in BENCHMARK_DIRS:
    raise ValueError(
        f"Unknown LIFT_FLOOR_BENCHMARK_TARGET={BENCHMARK_TARGET!r}; "
...
# ----------------------------------------------------------------------------
if IS_TETRALITH:
    BUDGET = os.environ.get("LIFT_FLOOR_BUDGET", "30m")
    MEMORY = os.environ.get("LIFT_FLOOR_MEMORY", "8G")
    INSTANCES_PER_DOMAIN = int(
        os.environ.get("LIFT_FLOOR_INSTANCES_PER_DOMAIN", "0")
    )
    INSTANCE_STEP = int(os.environ.get("LIFT_FLOOR_INSTANCE_STEP", "1"))
else:
    BUDGET = os.environ.get("LIFT_FLOOR_BUDGET", "5m")
    MEMORY = os.environ.get("LIFT_FLOOR_MEMORY", "8G")
    INSTANCES_PER_DOMAIN = int(
        os.environ.get("LIFT_FLOOR_INSTANCES_PER_DOMAIN", "1")
    )
    INSTANCE_STEP = int(os.environ.get("LIFT_FLOOR_INSTANCE_STEP", "3"))

LOCAL_PROCESSES = int(os.environ.get("LIFT_FLOOR_PROCESSES", "2"))
...
_domains_filter = os.environ.get("LIFT_FLOOR_DOMAINS")
if _domains_filter:
    requested = [d.strip() for d in _domains_filter.split(",") if d.strip()]
    missing = [d for d in requested if d not in SUITE]
    if missing:
        raise ValueError(
            f"LIFT_FLOOR_DOMAINS includes unknown: {missing}; "
            f"available: {SUITE}"
        )
    SUITE = requested
print(f"[lift-floor] {len(SUITE)} domains under {BENCHMARKS_DIR}")
...
# LIFT_FLOOR_WALL_TIME_FLOOR if a future scope pushes the estimate higher.
...
if IS_TETRALITH:
    ...
    print(
        f"[lift-floor] {NUM_RUNS} runs, {RUNS_PER_TASK} runs/array-task; "

#!/usr/bin/env python3
"""Lazy preferred-operator x queue-scheduling ablation at slope 48.

Four first-solution configurations cross preferred queues (absent/present)
with multi-heuristic queue scheduling (per-sweep/per-depth). All use lazy FF
and landmark-sum evaluation, fixed slope 48, no boosting, no pruning heuristic,
and no anytime improvement.
...
def detect_cluster():
    forced = os.environ.get("LPORR_CLUSTER", "").strip().lower()
    if forced:
        if forced not in ("tetralith", "local"):
            raise ValueError("LPORR_CLUSTER must be tetralith or local")
        return forced
    if (
...
if IS_TETRALITH:
    BUDGET = os.environ.get("LPORR_BUDGET", "15m")
    MEMORY = os.environ.get("LPORR_MEMORY", "6G")
    INSTANCE_STEP = int(os.environ.get("LPORR_INSTANCE_STEP", "1"))
else:
    BUDGET = os.environ.get("LPORR_BUDGET", "180s")
    MEMORY = os.environ.get("LPORR_MEMORY", "4G")
    INSTANCE_STEP = int(os.environ.get("LPORR_INSTANCE_STEP", "10"))
INSTANCES_PER_DOMAIN = int(os.environ.get("LPORR_INSTANCES_PER_DOMAIN", "0"))
LOCAL_PROCESSES = int(os.environ.get("LPORR_PROCESSES", "2"))
...
domain_filter = os.environ.get("LPORR_DOMAINS")
SUITE = (
...
def build_suite():
    index_text = os.environ.get("LPORR_INSTANCE_INDEX")
    instance_index = int(index_text) if index_text else None
    if instance_index is not None and instance_index < 1:
        raise ValueError("LPORR_INSTANCE_INDEX must be >= 1")
    if INSTANCE_STEP <= 0:
        raise ValueError("LPORR_INSTANCE_STEP must be positive")
    tasks = []
    for domain in SUITE:
...
TASKS = build_suite()
print(
    f"[lazy-po-rr] cluster={CLUSTER}, {len(SUITE)} domains, "
...
SEARCH_TEMPLATES = {
    "lazy-sweep-no-po": ff_lm(
        f"lazy_multi_triangle(evals=[hff, hlm], slope={SLOPE}, schedule=sweep)"
    ),
    "lazy-depth-no-po": ff_lm(
        f"lazy_multi_triangle(evals=[hff, hlm], slope={SLOPE}, schedule=depth)"
    ),
    "lazy-sweep-po": ff_lm(
        "lazy_multi_triangle(evals=[hff, hlm], "
        f"preferred_evals=[hff, hlm], slope={SLOPE}, schedule=sweep)"
    ),
    "lazy-depth-po": ff_lm(
        "lazy_multi_triangle(evals=[hff, hlm], "
        f"preferred_evals=[hff, hlm], slope={SLOPE}, schedule=depth)"
    ),
}

config_filter = os.environ.get("LPORR_CONFIGS")
if config_filter:
...
print(f"[lazy-po-rr] configs: {[name for name, _ in CONFIGS]}")
...
if IS_TETRALITH:
    ...
    floor = duration_to_seconds(
        os.environ.get("LPORR_WALL_TIME_FLOOR", "10:00:00")
    )
    ENV.time_limit_per_task = seconds_to_hms(max(estimate, floor))
    print(
        f"[lazy-po-rr] {num_runs} runs, {runs_per_task} runs/array task, "
...
stem = Path(__file__).stem + os.environ.get("LPORR_EXPERIMENT_SUFFIX", "")

#!/usr/bin/env python3
"""Random-start and fixed-window triangle sweeps.

Random start and windows 1,5,10,100,1000, each eager/lazy and with/without
depth-stratified preferred operators. Default: 48 configurations per task,
using sweep and depth scheduling, one slope and one reproducible random seed. No batching
or global preferred queue variants.

Task selection, budgets, Slurm settings, parsers and reports follow the
2026-09-15 triangle-batches experiment. Tetralith: full Autoscale Agile STRIPS,
15m search time / 6G overall memory. Local: every tenth task, 180s / 4G.
Overrides use RRTW_; see README.md.
"""
...
SLOPE = int(os.environ.get("RRTW_SLOPE", "1"))
if SLOPE < 1:
    raise ValueError("RRTW_SLOPE must be positive")
WINDOWS = (1, 5, 10, 100, 1000)
RANDOM_SEED = int(os.environ.get("RRTW_RANDOM_SEED", "42"))
if RANDOM_SEED < 0:
    raise ValueError("RRTW_RANDOM_SEED must be nonnegative for reproducible runs")
SCHEDULES = tuple(name.strip() for name in
                  os.environ.get("RRTW_SCHEDULES", "sweep,depth").split(",") if name.strip())
if not SCHEDULES or len(set(SCHEDULES)) != len(SCHEDULES) or any(
    name not in ("sweep", "depth") for name in SCHEDULES
):
    raise ValueError("RRTW_SCHEDULES must contain sweep and/or depth without duplicates")


def detect_cluster():
    forced = os.environ.get("RRTW_CLUSTER", "").strip().lower()
    if forced:
        if forced not in ("tetralith", "local"):
            raise ValueError("RRTW_CLUSTER must be tetralith or local")
        return forced
    if (
...
if IS_TETRALITH:
    BUDGET = os.environ.get("RRTW_BUDGET", "15m")
    MEMORY = os.environ.get("RRTW_MEMORY", "6G")
    INSTANCE_STEP = int(os.environ.get("RRTW_INSTANCE_STEP", "1"))
else:
    BUDGET = os.environ.get("RRTW_BUDGET", "180s")
    MEMORY = os.environ.get("RRTW_MEMORY", "4G")
    INSTANCE_STEP = int(os.environ.get("RRTW_INSTANCE_STEP", "10"))
INSTANCES_PER_DOMAIN = int(os.environ.get("RRTW_INSTANCES_PER_DOMAIN", "0"))
LOCAL_PROCESSES = int(os.environ.get("RRTW_PROCESSES", "2"))
...
domain_filter = os.environ.get("RRTW_DOMAINS")
SUITE = (
...
def build_suite():
    index_text = os.environ.get("RRTW_INSTANCE_INDEX")
    instance_index = int(index_text) if index_text else None
    if instance_index is not None and instance_index < 1:
        raise ValueError("RRTW_INSTANCE_INDEX must be >= 1")
    if INSTANCE_STEP <= 0:
        raise ValueError("RRTW_INSTANCE_STEP must be positive")
    tasks = []
    for domain in SUITE:
...
TASKS = build_suite()
print(
    f"[triangle-windows] cluster={CLUSTER}, "
...
config_filter = os.environ.get("RRTW_CONFIGS")
if config_filter:
...
print(f"[triangle-windows] configs: {[n for n, _ in CONFIGS]}")
...
if IS_TETRALITH:
    ...
    floor = duration_to_seconds(
        os.environ.get("RRTW_WALL_TIME_FLOOR", "10:00:00")
    )
    ENV.time_limit_per_task = seconds_to_hms(max(estimate, floor))
    print(
        f"[triangle-windows] {num_runs} runs, "
...
stem = Path(__file__).stem + os.environ.get("RRTW_EXPERIMENT_SUFFIX", "")

#!/usr/bin/env python3
"""Triangle search with one global FIFO preferred-operator queue.

Draft: currently includes eager/lazy fixed slope 48 only. Adaptive modes
await clarification of the per-step versus per-sweep update rule.

Uses the same FF + landmark-sum guidance, task suite, Tetralith resources,
15m search time / 6G overall memory limits, parsers, and reports as the
triangle-batches experiment. Environment overrides use GPTS_.
"""
...
def detect_cluster():
    forced = os.environ.get("GPTS_CLUSTER", "").strip().lower()
    if forced:
        if forced not in ("tetralith", "local"):
            raise ValueError("GPTS_CLUSTER must be tetralith or local")
        return forced
    if (
...
if IS_TETRALITH:
    BUDGET = os.environ.get("GPTS_BUDGET", "15m")
    MEMORY = os.environ.get("GPTS_MEMORY", "6G")
    INSTANCE_STEP = int(os.environ.get("GPTS_INSTANCE_STEP", "1"))
else:
    BUDGET = os.environ.get("GPTS_BUDGET", "180s")
    MEMORY = os.environ.get("GPTS_MEMORY", "4G")
    INSTANCE_STEP = int(os.environ.get("GPTS_INSTANCE_STEP", "10"))
INSTANCES_PER_DOMAIN = int(os.environ.get("GPTS_INSTANCES_PER_DOMAIN", "0"))
LOCAL_PROCESSES = int(os.environ.get("GPTS_PROCESSES", "2"))
...
domain_filter = os.environ.get("GPTS_DOMAINS")
SUITE = (
...
def build_suite():
    index_text = os.environ.get("GPTS_INSTANCE_INDEX")
    instance_index = int(index_text) if index_text else None
    if instance_index is not None and instance_index < 1:
        raise ValueError("GPTS_INSTANCE_INDEX must be >= 1")
    if INSTANCE_STEP <= 0:
        raise ValueError("GPTS_INSTANCE_STEP must be positive")
    tasks = []
    for domain in SUITE:
...
TASKS = build_suite()
print(
    f"[global-preferred-triangle] cluster={CLUSTER}, "
    f"{len(SUITE)} domains, {len(TASKS)} tasks, fixed_slope={FIXED_SLOPE}"
)
...
for timing, plugin in SEARCH_VARIANTS:
    name = f"triangle-{timing}-global-po-ff-lm-fixed-s48"
    SEARCH_TEMPLATES[name] = ff_lm(
        f"{plugin}(evals=[hff, hlm], preferred_evals=[hff, hlm], "
        f"schedule=sweep, global_preferred=true, slope={FIXED_SLOPE})"
    )

config_filter = os.environ.get("GPTS_CONFIGS")
if config_filter:
...
print(f"[global-preferred-triangle] configs: {[n for n, _ in CONFIGS]}")
...
if IS_TETRALITH:
    ...
    floor = duration_to_seconds(
        os.environ.get("GPTS_WALL_TIME_FLOOR", "10:00:00")
    )
    ENV.time_limit_per_task = seconds_to_hms(max(estimate, floor))
    print(
        f"[global-preferred-triangle] {num_runs} runs, "
...
stem = Path(__file__).stem + os.environ.get("GPTS_EXPERIMENT_SUFFIX", "")
